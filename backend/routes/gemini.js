...
// --- MULTIMODAL QUIZ GENERATION SERVICE ---
router.post('/generateQuizFromFile', async (req, res) => {
    try {
        ...
        let prompt = `Analyze the provided document (which may be a PDF, an image, or handwritten notes) and generate a set of ${count} multiple-choice quiz questions based on its core contents.
        Focus on testing key concepts, definitions, and applications found directly in the file.
        ...
        res.json({ quizSet: cleanedResponse });
    } catch (error) {
...
router.post('/generateFlashcardsFromFile', async (req, res) => {
    try {
        ...
        let prompt = `Analyze the provided document (which may be a PDF, an image, or handwritten notes) and generate a list of flashcards covering all the key concepts, definitions, and formulas inside it.
        Each flashcard should have a 'front' (a question or term) and a 'back' (the answer or definition).
        RETURN ONLY RAW JSON. Do not wrap in markdown or code blocks.`;
        ...
        console.error("Error in generateFlashcardsFromFile:", error);
        res.status(500).json({ error: error.message });
    }
});
