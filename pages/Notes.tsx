...
const StudyPlanView: React.FC<{
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const parsedData = parseStudyPlanPayload(planJson);

      const inferTaskType = (task: any): 'note' | 'quiz' | 'study-room' | 'review' => {
        const directType = String(task?.type || '').toLowerCase();
        if (directType === 'note' || directType === 'quiz' || directType === 'study-room' || directType === 'review') {
          return directType;
        }

        const hint = `${task?.technique || ''} ${task?.task || ''} ${task?.title || ''}`.toLowerCase();
        if (hint.includes('quiz') || hint.includes('test')) return 'quiz';
        if (hint.includes('group') || hint.includes('collab') || hint.includes('room')) return 'study-room';
        if (hint.includes('review') || hint.includes('revise')) return 'review';
        return 'note';
      };

      const rawDays = Array.isArray(parsedData)
        ? parsedData
        : (Array.isArray(parsedData?.days) ? parsedData.days : (Array.isArray(parsedData?.schedule) ? parsedData.schedule : []));

      const days = rawDays.map((d: any, idx: number) => ({
        day: Number(d?.day) || (idx + 1),
        tasks: (Array.isArray(d?.tasks) ? d.tasks : []).map((t: any) => ({
          title: String(t?.title || t?.task || 'Study Task'),
          description: String(t?.description || t?.task || t?.technique || ''),
